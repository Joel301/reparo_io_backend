...
router.post("/login",async (req,res,next)=> {
    ...
    if (email && password){
        ...
        if (data) {        
         ...
         { return res.status(400).send("Error de credenciales")}
    }
    else { return res.status(401).send("Login Error")}
    
})
router.get("/login", (req,res,next)=>{
    const {userId} = req.cookies;
    console.log(userId);
    if (userId) { getUserById(userId).then((user) => { res.send(user) })}
